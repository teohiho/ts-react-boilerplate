...
const Tab1 = () => (
	<>
		<Button onClick={async () => {
			try {
				const data = await tifl.patient.get({ max_results: 20, page: 7 })
		...
		<Button onClick={async () => {
			try {
				const data = await tifl.organization.get({ max_results: 20, page: 7 })
		...
		<Button onClick={async () => {
			try {
				const data = await tifl.encounter.get({ max_results: 20, page: 7 })
				console.log('DATA', data.data)
			} catch (err) {
				console.log('ERR', err)
			}
